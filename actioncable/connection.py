...
class Connection:
    ...
    def connect(self, origin=None):
        ...
        self.ws_thread = threading.Thread(
            name="APIConnectionThread_{}".format(uuid.uuid1()), target=self._run_forever
        )
        ...
        while self.auto_reconnect:
            try:
                ...
                self.websocket = websocket.WebSocketApp(
                    ...
                    cookie=self.cookie,
                    header=self.header,
                    on_message=lambda socket, message: self._on_message(
                        socket, message
                    ),
                ...
                self.websocket.run_forever(
                    ping_interval=5, ping_timeout=3, origin=self.origin
                )

                time.sleep(2)
            except Exception as exc:
                self.logger.error(
                    "Connection loop raised exception. Exception: %s", exc
                )
    ...
    def connected(self):
        ...
        return (
            ...
            and self.websocket.sock.connected
        )
